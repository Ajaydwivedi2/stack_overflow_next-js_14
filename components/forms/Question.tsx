...
function Question({ type, questionDetails, mongoUserId }: Props) {
  ...
  const handleTagRemove = (tag: string, field: any) => {
    const newTagArr: string[] = field.value.filter(
      (item: string) => item !== tag
    );
    form.setValue("tags", newTagArr);
  };
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex w-full flex-col">
              ...
              <FormDescription className="body-regular mt-2.5 text-light-500">
                Introduce to the problem and expand on what you put in the
